...
const LoginFunction = async (email, password, onStatusUpdate) => {
  try {
    ...
    onStatusUpdate("1. Getting CSRF token...", "success");
    await fetch(`${API_BASE_URL}/users/extension/login/`, {
      method: "GET",
      credentials: 'include',
    });

    // Now make the login request with the CSRF cookie
    onStatusUpdate("2. Sending login request...", "success");
    ...
    onStatusUpdate("3. Processing response...", "success");
    ...
    onStatusUpdate("4. Login successful!", "success");
    setTimeout(() => onStatusUpdate("", "clear"), 5000);
    return data;
  } catch (error) {
